(ns qat.core
  (:require [qat.llm-interface :as llm]
            ...
            [qat.retriever :as retriever] ; New require
            [clojure.string :as str]
            [clojure.java.io :as io]
            [clojure.data.json :as json])
  (:gen-class))

;; --- Agent Definitions (Updated to use retriever context) ---
...
(defn generate-questions [model-name sample-article-text aspect num-questions]
...
(defn generate-answer [model-name retrieved-context question]
...
(defn run-critic [model-name retrieved-context question answer]
...
(defn run-rag-q-and-a-session
  [q-model a-model c-model corpus sample-article-text aspect num-questions]
  (if-let [questions (generate-questions q-model sample-article-text aspect num-questions)]
    (do
      ...
      (doall
       (for [question questions]
         (let [retrieved-context (retriever/search-corpus corpus question)
               answer (generate-answer a-model retrieved-context question)
               critique (run-critic c-model retrieved-context question answer)]
           {:question question :answer answer :critique critique}))))
    (do (println (str "<- Agent failed to generate questions for aspect '" aspect "'.")) [])))
    ...
    (let [session-futures (doall
                           (for [[index aspect] (map-indexed vector aspects)]
                             (future
                               ...
                               (let [qa-pairs (run-rag-q-and-a-session GUEST_MODEL EXPERT_MODEL CRITIC_MODEL corpus sample-article-text aspect 2)
...
(defn -main
  ...
  (if-let [corpus-dir (first args)]
    (if (.isDirectory (clojure.java.io/file corpus-dir))
      (let [corpus (retriever/load-corpus corpus-dir)
            sample-article-text (:text (first corpus)) ; Use the first doc for theme generation
